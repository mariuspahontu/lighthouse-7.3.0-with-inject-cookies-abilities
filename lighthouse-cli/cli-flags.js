...
function getFlags(manualArgv, options = {}) {
  ...
  Example: --output-path=./lighthouse-results.html`,
      ...
      .options({
        ...
        'extra-cookies': {
          coerce: coerceExtraCookies,
          describe: 'Set extra HTTP Cookies to pass with request',
        },
...
/**
 * `--extra-cookies` comes in as a JSON string or a path to a JSON string, but the flag value
 * needs to be the parsed object. Load file (if necessary) and returns the parsed object.
 * @param {unknown} value
 * @return {LH.SharedFlagsSettings['extraCookies']}
 */
function coerceExtraCookies(value) {
  // TODO: this function does not actually verify the object type.
  if (value === undefined) return value;
  if (typeof value === 'object') return /** @type {LH.SharedFlagsSettings['extraCookies']} */ (value);
  if (typeof value !== 'string') {
    throw new Error(`Invalid value: Argument 'extra-headers' must be a string`);
  }
